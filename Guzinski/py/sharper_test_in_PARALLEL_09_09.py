...
def runSharpi(highResFilename, lowResFilename, lowResMaskFilename, cv, movWin, regrat, outputFilename, useDecisionTree = True):
    ...
    dtOpts =     {"perLeafLinearRegression":    True,
                    "linearRegressionExtrapolationRatio": regrat}
    ...
    outFile = utils.saveImg(outImage.GetRasterBand(1).ReadAsArray(),
                            ...
                            outputFilename)#f'{os.path.split(outputFilename)[0]}/Values/{os.path.split(outputFilename)[1]}')
    residualFile = utils.saveImg(residualImage.GetRasterBand(1).ReadAsArray(),
                                ...
                                outputFilename)#f'{os.path.split(outputFilename)[0]}/Residuals/{os.path.split(outputFilename)[1]}_resid{os.path.splitext(outputFilename)[1]}')
...
Tiles_X = [[70],[70,71],[70,71,70],[70,71,70,70]]
Tiles_Y = [[44],[44,44],[44,44,45],[44,44,45,45]]
...
for Tile_X, Tile_Y in zip(Tiles_X, Tiles_Y):
        ...
        if os.path.exists(f'{temp_dump_fold}S2_{date}'):
            ...
            for comp_stat in ['minVZA', 'maxLST']:
                ...
                for k, v in band_dict.items():
                    ...
                    gdal.BuildVRT(highRes_path, [S2_path], separate=True) # , slope_path, aspect_path, incid_path
                    # maskVRT_water(highRes_path)
                    # highRes_files.append(f'{highRes_path.split('.')[0]}_watermask.tif')
                    highRes_files.append(f'{highRes_path}')
                    ...
                    # maskVRT(f'{highRes_path}', mask)
                    # # highRes_files.append(f'{highRes_path.split('.')[0]}_S2_agromask.tif')
                    lowRes_files.append(f'{temp_dump_fold}Daily_LST_{comp_stat}_{year}_{month}_{band:02d}.tif')
    ...
    for idx, highResFilename in enumerate(highRes_files):
        ...
        #                 None,
        #                 0, 15, 0.25,
        #                 f'{outFolder}{'_'.join(highResFilename.split('.')[0].split('_')[2:6])}_mv15_cv0_reg025_noMask_X{Tiles_X[0]}_Y{Tiles_Y[0]}.tif'])
