...
def run_evapi(year, month, day, comp, sharp, s2Mask, lstMask):

       # set output path
    tempOut = '/data/Aldhani/eoagritwin/et/Auxiliary/trash/3X3/'
    ...
    outPath = f'{tempOut}{sharp}_{comp}_{year}_{month}_{day:02d}_{lstMask}_{s2Mask}/'
    storPath_c = f'{outPath}{comp}_{year}_{month}_{day}_mvwin{mvwin}_cv{cv}_regrat{regrat}_{lstMask}_{s2Mask}_{sharp}_ET_Canopy_calc.tif'
    storPath_s = f'{outPath}{comp}_{year}_{month}_{day}_mvwin{mvwin}_cv{cv}_regrat{regrat}_{lstMask}_{s2Mask}_{sharp}_ET_Soil_calc.tif'

    storPath_c_f = f'{outPath}{comp}_{year}_{month}_{day}_mvwin{mvwin}_cv{cv}_regrat{regrat}_{lstMask}_{s2Mask}_{sharp}_ET_Canopy_func.tif'
    storPath_s_f = f'{outPath}{comp}_{year}_{month}_{day}_mvwin{mvwin}_cv{cv}_regrat{regrat}_{lstMask}_{s2Mask}_{sharp}_ET_Soil_func.tif'
    ...
    sharp_pathbase = f"/data/Aldhani/eoagritwin/et/Sentinel3/LST/LST_values/sharpened_3X3/{comp}/{year}/{month}/{day:02d}/{lstMask}/{s2Mask}/Values/"
    s2_pathbase = f'/data/Aldhani/eoagritwin/et/Sentinel3/LST/LST_values/tempDump3X3/TEST3X3NEW'
    ...
    for path in era5_path_list:
        ...
        if '100m_u_component_of_wind' in path:
            ...
            try:
                ...
                with open(f'{tempOut}ERROR_{comp}_{year}_{month}_{day}_mvwin{mvwin}_cv{cv}_regrat{regrat}_{lstMask}_{s2Mask}_{sharp}_ET.log', 'a') as f:
                    f.write(f'{e}')
                return
    ...
    # ssrd_mean_calc_20 = ssrd_mean.reshape((1500,1500))
    ...
    npTOdisk(ssrd_mean_calc_20, LST_file, f'{outPath}SSRD_mean_calc.tif', bands = 1)
    npTOdisk(ssrd_mean_func_20, LST_file, f'{outPath}SSRD_mean_func.tif', bands = 1)
...
if __name__ == '__main__':
    ...
    Parallel(n_jobs=ncores)(delayed(run_evapi)(job[0], job[1], job[2], job[3], job[4], job[5], job[6]) for job in joblist)
