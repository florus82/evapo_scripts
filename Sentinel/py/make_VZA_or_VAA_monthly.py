...
files = sorted(getFilelist(f"/data/Aldhani/eoagritwin/et/Sentinel3/{mod}/raw", '.nc'))

for year in [i for i in range(2020,2025,1)]:
    outPath = f"/data/Aldhani/eoagritwin/et/Sentinel3/{mod}/monthly_tiff_values/{year}/"
    ...
    for month in [f'{i:02d}' for i in range(1,13)]:
        filename = f"{mod}_{year}_{month}.tif"
